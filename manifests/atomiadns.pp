...
add_nameserver_group:
	...
	if $ssl_enabled {
		...
		$atomiadns_conf = generate("/etc/puppet/modules/atomiadns/files/generate_conf.sh", $agent_user, $agent_password, $hostname, $dns_url, "ssl")

	} 
	else {
		$atomiadns_conf = generate("/etc/puppet/modules/atomiadns/files/generate_conf.sh", $agent_user, $agent_password, $hostname, $dns_url, "nossl")
	}
