class atomia::apache (
	...
	if $atomia_linux_software_auto_update {
		if $should_have_pa_apache == 1 { package { atomia-pa-apache: ensure => latest } }
	        package { atomiastatisticscopy: ensure => latest }
	} else {
		if $should_have_pa_apache == 1 { package { atomia-pa-apache: ensure => present } } 
	...
	$settings_content = generate("/etc/puppet/modules/apache_agent/files/settings.cfg.sh", $username, $password, $ssl_generate_var)
